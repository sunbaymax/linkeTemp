                ...
                { 
                   ...
                   { 
                       if(strstr($val2['text'],"[") && strstr($val2['text'],"]") && ($val2['label_category']!= '000000000'))  //8个0是净化器
                       {
                           //1th
                           if ($val['onlineNum'] == 1) 
        				       echo '<div class="in_div_MarkA" onclick="shuju('.$val2['id'].')" bianhao='.$val2['id'].'>';  //P3-Begin-牌子
                           else 
...
/*点击图片能进入设备 能进入设备导出实际数据模块sunchangbo*/
function shuju(id){
	window.location.href="../../index.php/member/devices?bianhao="+id;
}
