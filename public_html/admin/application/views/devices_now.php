                ...
                { 
                   ...
                   { 
                       if(strstr($val2['text'],"[") && strstr($val2['text'],"]") && ($val2['label_category']!= '000000000'))  //8个0是净化器
                       {
                           //1th
                           if ($val['onlineNum'] == 1) 
        				       echo '<div class="in_div_MarkA" onclick="shuju('.$val2['id'].')" id="' . $val2['id'] . '_' . $val2['label_category'] . '_' . $val['text'] . '_' . $val2['text'] .$val['host_code'].'_'.$val['host_alias'].  '" >';  //P3-Begin-牌子
                           else 
                               echo '<div class="in_div_MarkB" onclick="shuju('.$val2['id'].')" id="' . $val2['id'] . '_' . $val2['label_category'] . '_' . $val['text'] . '_' . $val2['text'] .$val['host_code'].'_'.$val['host_alias'].  '">';  //P3-Begin-牌子 
...
//$('.in_div_MarkA').on("click",function(){
//  var labelsname=	$(this).attr('id')
//	window.sessionStorage.setItem("nums2", labelsname);
//})
/*点击图片能进入设备 能进入设备导出实际数据模块*/
function shuju(id){
	window.sessionStorage.setItem("nums", id);
	window.location.href="../../index.php/member/devices";
}
